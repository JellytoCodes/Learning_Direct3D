...
void BoxApp::OnResize()
{
	...
	// 창의 크기가 변경되므로 종횡비를 갱신하고 투영 행렬을 다시 계산
...
void BoxApp::Update(const GameTimer& gt)
{
	...
	// 최신 worldViewProj 행렬로 상수 버퍼를 갱신
...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	// 이러한 대기는 비효율적이므로 렌더링 코드를 조직화해서 프레임마다 대기할 필요가 없게 만들어야 한다.
	FlushCommandQueue();
}
...
void BoxApp::OnMouseMove(WPARAM btnState, int x, int y)
{
	if ((btnState & MK_LBUTTON) != 0)
	{
		...
		// 마우스 입력에 기초해 각도를 갱신한다.
		// 이에 의해 카메라가 상자를 중심으로 공전하게 된다.
		mTheta += dx;
		mPhi += dy;
...
void BoxApp::BuildRootSignature()
{
	...
	// 루트 서명은 셰이더 프로그램의 기대하는 자원들을 정의한다.
