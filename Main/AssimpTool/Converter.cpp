...
void Converter::ReadModelData(aiNode *node, int32 index, int32 parent)
{
	...
	// 재귀 함수
	for (uint32 i = 0 ; i < node->mNumChildren ; i++) ReadModelData(node->mChildren[i], _bones.size(), index);
}
...
void Converter::ReadMeshData(aiNode *node, int32 bone)
{
	...
	for (uint32 i = 0 ; i < node->mNumMeshes; i++)
	{
		...
		for (uint32 v = 0 ; v < srcMesh->mNumVertices ; v++)
		{
			...
			// UV
			if (srcMesh->HasTextureCoords(0))		::memcpy(&vertex.uv, &srcMesh->mTextureCoords[0][v], sizeof(Vec2));

			// Normal
			if (srcMesh->HasNormals())					::memcpy(&vertex.normal, &srcMesh->mNormals[v], sizeof(Vec3));
		...
		for (uint32 f = 0 ; f < srcMesh->mNumFaces ; f++)
		{
			...
			for (uint32 k = 0 ; k < face.mNumIndices ; k++)	mesh->indices.push_back(face.mIndices[k] + startVertex);
		}
	}
...
void Converter::ReadMaterialData()
{
	for(uint32 i = 0; i < _scene->mNumMaterials; i++)
	{
		...
		srcMaterial->Get(AI_MATKEY_SHININESS, material->specular.w);	// Specular의 세기(w)에 대한 정보를 가져올 수 있는 키이다.
...
void Converter::WriteMaterialData(wstring finalPath)
{
	...
	const string folder = path.parent_path().string();
	...
	for(const shared_ptr<asMaterial> material : _materials)
	{
...
string Converter::WriteTexture(string saveFolder, string file)
{
	...
	if(srcTexture)
	{
		...
		{
			shared_ptr<FileUtils> infile = make_shared<FileUtils>();
			infile->Open(Utils::ToWString(pathStr), FileMode::Write);
			infile->Write(srcTexture->pcData, srcTexture->mWidth);
		}
		else
		{
