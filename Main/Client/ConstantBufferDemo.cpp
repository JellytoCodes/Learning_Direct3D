...
void ConstantBufferDemo::Init()
{
	...
	GeometryHelper::CreateQuad(_geometry, Color(0.f, 1.f, 1.f, 1.f));
...
void ConstantBufferDemo::Update()
{
	...
	if (INPUT->GetButton(KEY_TYPE::A)) _translation.x -= 1.f * dt;
	if (INPUT->GetButton(KEY_TYPE::D)) _translation.x += 1.f * dt;
	if (INPUT->GetButton(KEY_TYPE::W)) _translation.y += 1.f * dt;
	if (INPUT->GetButton(KEY_TYPE::S)) _translation.y -= 1.f * dt;
