...
void DiffuseDemo::Update()
{
	...
	_shader->GetVector("LightDiffuse")->SetFloatVector(reinterpret_cast<float*>(&lightDiffuse));
...

	{
		...
		_shader->GetVector("MaterialDiffuse")->SetFloatVector(reinterpret_cast<float*>(&materialDiffuse));
		_obj->Update();	
	}
	{
		Vec4 materialDiffuse(1.f);
