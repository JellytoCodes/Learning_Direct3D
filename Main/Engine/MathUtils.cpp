...
Point3D MathUtils::ClosestPoint(const Line3D& line, const Point3D& point)
{
	...
	t = fmaxf(t, 0.f); // Clamp to 0
	t = fminf(t, 1.f); // Clamp to 1
...
bool MathUtils::AABBAABB(const AABB3D& aabb1, const AABB3D& aabb2)
{
	...
	Point3D aMax = AABB3D::GetMin(aabb1);
	Point3D bMin = AABB3D::GetMin(aabb2);
	Point3D bMax = AABB3D::GetMin(aabb2);
...
bool MathUtils::AABBOBB(const AABB3D& aabb, const OBB3D& obb)
{
	...
	for (int i = 0; i < 15; ++i)
	{
		if (OverlapOnAxis(aabb, obb, test[i]) == false) return false; // Seperating axis found
	}
...
bool MathUtils::OBBOBB(const OBB3D& obb1, const OBB3D& obb2)
{
	...
	for (int i = 0; i < 15; ++i)
	{
		if (OverlapOnAxis(obb1, obb2, test[i]) == false) return false; // Seperating axis found
	}
...
{
	Vec3 aabbMin = AABB3D::GetMin(aabb);
	Vec3 aabbMax = AABB3D::GetMax(aabb);
...
bool MathUtils::PointInTriangle(const Point3D& p, const Triangle3D& t)
{
	...
	if (normPBC.Dot(normPAB) < 0.0f)		return false;
...
bool MathUtils::Raycast(const Triangle3D& triangle, const Ray3D& ray, float& distance)
{
	...
	float t = 0;
	if (Raycast(plane, ray, OUT t) == false)	return false;
