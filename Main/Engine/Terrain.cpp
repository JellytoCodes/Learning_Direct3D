...
void Terrain::Create(float sizeX, float sizeZ, shared_ptr<Material> material)
{
	_sizeX = sizeX;
	_sizeZ = sizeZ;
	...
	if (go->GetMeshRenderer() == nullptr) go->AddComponent(make_shared<MeshRenderer>());
...
bool Terrain::Pick(float screenX, float screenY, Vec3& pickPos, float& distance)
{
