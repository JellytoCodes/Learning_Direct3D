...
void StructuredBuffer::CreateInput()
{
	...
	if (_inputData != nullptr)	CHECK(DEVICE->CreateBuffer(&desc, &subResource, _input.GetAddressOf()));
	else						CHECK(DEVICE->CreateBuffer(&desc, nullptr, _input.GetAddressOf()));
...
void StructuredBuffer::CreateOutput()
{
	...
	desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED; // RAW_BUFFER
